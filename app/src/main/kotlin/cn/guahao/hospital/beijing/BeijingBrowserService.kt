...
/** Private IPC accepts the fixed public query contract only, with bounded concurrency. */
abstract class BeijingBrowserService : Service() {
    ...
    private val endpoint by lazy { Messenger(object : Handler(Looper.getMainLooper()) {
        override fun handleMessage(message: Message) {
            ...
            if (message.what != QUERY || requests.containsKey(id)) return
            ...
            val job = scope.launch(start = CoroutineStart.LAZY) {
                ...
                try {
                    ...
                    val reply = BeijingBrowserRuntime.get(this@BeijingBrowserService, channel).execute(query)
    ...
    companion object { const val QUERY = 1; const val CANCEL = 2; const val RESULT = 3 }
}
