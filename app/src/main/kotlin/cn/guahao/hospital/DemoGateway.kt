...
class DemoGateway(private val store: TaskStore) : BookingGateway {
    ...
    override suspend fun lock(task: BookingTask, candidate: Candidate): LockReply { check(task.demo); return LockReply.Accepted }
    override suspend fun querySubmission(patient: PatientRef): AsyncReply {
        val r = store.all().firstOrNull { it.task.demo && it.attempt != null && it.order == null } ?: return AsyncReply.Pending
    ...
    override suspend fun orders(patient: PatientRef, from: LocalDate, to: LocalDate): List<OrderSnapshot> = store.all().filter {
        it.task.demo && it.task.condition.patient == patient && it.attempt != null && it.task.condition.visitDate in from..to
    }.map { r ->
    ...
    override suspend fun paymentState(patient: PatientRef, orderNo: String) = if (store.all().any {
        it.order?.orderNo == orderNo && it.order?.phase == OrderPhase.INSURANCE_PAID
