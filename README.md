...
用户提前设置就诊人、科室、医生、就诊日期和放号时间。App 在放号窗口内按条件尝试预约，取得待支付订单后停止查号并通知用户付款。任务最长运行时长默认 30 分钟，可配置；付款时限以医院订单为准。

## 当前状态

项目处于 v0.1 需求与数据接入验证阶段，尚未创建 Android 工程，没有可安装的 APK。
...
- 待验证：服务号首次认证与失效恢复、异常提交恢复、独立支付流水，以及目标手机的定时唤醒、锁屏网络和后台运行。

支付偏好为优先医保移动支付。官方结果列表的缴费入口省略医保选择；锁号后的医保初始化与手机小程序交接尚未验证，医保不可用时提示用户处理，不自动改为全额支付。

这些结论来自单医院的有限验证，不代表接口长期可用，也不构成其他真实预约的授权。具体实现方案仍待评审。
...
仓库保留项目文档和脱敏技术证据。登录凭据、患者资料、原始订单明细、本机代理会话、飞书发布草稿、通知回执和修改前备份留在本地，不纳入提交。现阶段的验证材料不是可直接运行的自动挂号程序。
