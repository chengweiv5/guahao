...
用户已明确需要同时挂多家医院。当前实际适配器仍只有佑安服务号，执行器也仍有单任务限制；本轮修复了“新增连接被任务阻塞”的错误，尚不代表多医院并行已实现。后续隔离与迁移方案见 [多医院连接与并行任务设计](docs/design/2026-09-25-multi-hospital-connections.md)。
