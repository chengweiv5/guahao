...
- Task 6：本机与模拟器验收完成；Mate 60 Pro（鸿蒙 4.2.0.223 / API 31）已安装，4 项存储/演示/UI 测试及实际付款提醒通过。首次短时测试被用户亮屏干扰；排除该干扰后复验，出现华为 LOCKSCREEN 拦截与 App 冻结，宿主等待超时。待用户开启华为三项后台许可后继续复验；真实独立会话导入、长时间锁屏/Doze、手机重启和真实只读结果格式未执行。详细时序与边界见验收记录。
