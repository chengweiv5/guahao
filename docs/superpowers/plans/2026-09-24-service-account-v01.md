...
- Task 6：本机与模拟器验收完成；Mate 60 Pro（鸿蒙 4.2.0.223 / API 31）已安装，存储/演示/UI 共 4 项通过。用户开启华为三项后台许可后，短时全程灭屏演示连续两次通过（760ms / 887ms 模拟提交延迟），付款提醒已验证；共 5 个不同功能测试通过。测试改用独立虚构日期，避免先前演示订单正确触发防重复保护；生产 APK 未改动。真实独立会话导入、30 分钟以上锁屏/Doze、手机重启和真实只读结果格式未执行。详细时序与边界见验收记录。
