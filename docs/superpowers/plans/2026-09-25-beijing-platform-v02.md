...
- [x] 核验两个新增医院；官方页面观察与原生客户端请求分别出具结果，不把目录数当支持医院数。
...
### 认证与就诊人集成增量（2026-09-26）

- [x] 新增 `BeijingAccountClient.kt`：固定读取 `auth/user/get`、`auth/patient/list`、再次读取账户，拒绝核验期间账户变化。只返回必要身份、脱敏显示名与卡，不返回电话或完整姓名；所有对象和错误输出脱敏。
- [x] `BeijingBrowserService/Client/Runtime` 增加独立账户读取消息，公开查询消息仍拒绝私有端点，任意提交端点都不可表达。身份读取与公开查询共享运行时互斥。
- [x] 新增 `BeijingConnectionRepository.kt`：加密保存明确选择的医院、渠道、患者和卡；每次核验先撤销当前可用性，失败或账户变化不沿用旧可用状态，重启后的记录须重验。
- [x] 新增 `BeijingPatientPicker.kt` 并接入医院选择页：显式核验、患者与卡选择、保存选择及重试；不自动选择默认卡，不开放未完成的预约链路。
- [x] 定向测试账户变化、失效、未知字段、错误脱敏、渠道隔离、卡变化、重启/过期和取消；保留临时独立包数据运行真实 Kotlin 只读核验，只输出布尔和数量。
...
- 2026-09-26 真机增量：独立京通 WebView 官方手机号登录、重启后身份核对、患者/近期订单只读成功；实际 Kotlin 跨医院查询通过。新增医生/时段解析与可替换传输契约、WebView 查询实现；正式医院选择页已绑定独立进程公开查询及官方连接页，患者选择和自动任务未接通。114 3087 要求微信授权；近期订单为空，非空订单/同单核对仍缺样本。患者/订单只读授权已取得，不重新询问该范围。

- 2026-09-26 跨进程核验：后台首次初始化存在页面重载丢失、医生467；正常可见官方首页初始化再返回App后，两院标准查询通过。正式实现强制进程重建后再次进入连接页，不宣称锁屏或重启自动恢复通过。近期订单为空，历史样本与微信inspector关闭仍待用户回复。
