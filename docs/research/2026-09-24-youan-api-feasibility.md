...
医保补验：官方在锁号成功后通过 `POST /order/sxPayCN` 进入医保流程；“挂号结果查询”的缴费按钮直接进入全额支付入口。此前单笔验证不覆盖医保，当前无待付款单，本轮未初始化医保支付。详见[医保分支核对](2026-09-24-youan-medical-insurance-payment.md)。
...
**已有服务号会话下，真实单笔锁号、手机订单可见与医院已预约状态回查已经验证；医保付款交接与结算未验证。** 这不包含全新微信认证、自动续期或 Android 后台能力；独立支付流水与费用分摊未取得。早期查询样例不构成其他真实预约的授权。
...
以下为服务号接口。基础查询及会话已验证；23:33 起追加验证一次 `lockRegis`、`queryRegisStat` 和 `getRegisList`，同一订单返回“已预约”。支付操作由用户在手机完成，代理未调用支付接口。
...
| `/order/regisOrderSend`、`/order/sxPayCN`、`/regis/regisYbPayState` | 后续订单或支付流程 | 仅发现路径，未验证独立 App 内运行 |
...
2. **按用户最新要求，主闭环采用微信服务号接口锁号，再由用户在服务号“挂号结果查询”付款。** 该单笔路径已实测，但结果列表缴费省略医保选择，医保手机交接尚待验证；原互联网医院短信 API 保留为独立研究渠道；两渠道就诊人和订单可见性不保证同步。
