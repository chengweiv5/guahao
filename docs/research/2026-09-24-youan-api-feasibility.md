...
**已有服务号会话下，真实单笔锁号、手机订单可见与医院已预约状态回查已经验证。** 这不包含全新微信认证、自动续期或 Android 后台能力；独立支付流水与费用分摊未取得。早期查询样例不构成其他真实预约的授权。
...
2. **按用户最新要求，主闭环采用微信服务号接口锁号，再由用户在服务号“挂号结果查询”付款。** 该单笔路径已实测，原互联网医院短信 API 保留为独立研究渠道；两渠道就诊人和订单可见性不保证同步。
