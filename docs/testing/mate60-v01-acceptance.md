...
最新多医院并行实现及 APK 哈希以 [2026-09-25 并行执行验收](2026-09-25-multi-hospital-execution.md) 为准，下方保留历史轮次结果。
...
## 2026-09-25：医院连接状态与重新连接

本轮将“本地有会话”与“医院最近校验可用”分开。首次导入仍要求用户主动粘贴本人服务号页面，成功后加密保存；后续复用并显示最近校验时间。回前台/进入连接设置时限频复查，等待、执行或未决任务期间跳过额外自动复查；进程重启或上次成功超过 5 分钟时显示待校验，5 分钟不代表医院登录有效期。

断网、服务繁忙、普通重定向和异常响应保留凭据，提示重试；权限校验非成功码显示“挂号权限待核对”，不直接断言过期。HTTP 401 或重定向至微信 HTTPS OAuth 授权入口才持久标记“需重新连接”，不跟随重定向。重新导入后旧草稿不能直接启用，需要复用条件新建并核对当前就诊人；存在未决提交时仍要求先核对医院订单，不自动恢复任务。

验证：核心 **29/29**，debug 与 release 单元测试各 **16/16**；两个构建与 lint 通过（0 错误、9 项既有 UseKtx 提示）。Mate 60 Pro **4/4，6.643 秒**，模拟器 **4/4，9.284 秒**，覆盖连接页重试/重新导入、加密状态重载、旧草稿拦截和未决提交保护。测试均使用虚构数据，未发真实医院请求。新增任务保护测试初次受历史未决测试记录阻挡，随后隔离测试数据库与存储后通过；没有删除历史任务或放宽生产保护。
