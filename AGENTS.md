...
- 不要自动推送。开发、修改、验证和本地提交均不构成推送授权；只有用户明确要求推送时才可执行，授权仅适用于当次交付。
- 获得推送授权后，本项目（`/Users/bytedance/Code/guahao`）直接推送到 `origin/main`，使用 `git push origin HEAD:main`，不以创建或合并 MR 为前提。
